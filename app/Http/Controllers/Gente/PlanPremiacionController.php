...
class PlanPremiacionController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $conteosPorColaborador = $this->whereMonthIn(Aci::query(), 'fecha_incidente', $mesesSeleccionados)
        ...
        $preguntasRutaRaw = DB::table('evaluacion_owd_preguntas')
            ...
            ->tap(fn ($q) => $this->whereMonthIn($q, 'evaluaciones_owd.fecha_evaluacion', $mesesSeleccionados))
        ...
        $ausentismosRaw = DB::table('ausentismos')
            ->tap(fn ($q) => $this->whereMonthIn($q, 'fecha', $mesesSeleccionados))
        ...
        $eventosTripulacionRaw = DB::table('eventos_tripulacion')
            ->when(
                ...
                fn ($q) => $this->whereMonthIn($q, 'fecha', $mesesChecklist)->whereYear('fecha', $anio),
        ...
        foreach ($eventosTripulacionRaw as $row) {
            ...
            if ($row->rechazos !== null) {
                ...
                if ($docKey) {
                    $rechazosPorDocumento[$docKey][] = $val;
                }
                if ($nomKey) {
                    $rechazosPorNombre[$nomKey][] = $val;
                }
            }
            ...
            if ($row->adherencia_tiempo !== null) {
                ...
                if ($docKey) {
                    $adherenciaTiempoPorDocumento[$docKey][] = $val;
                }
                if ($nomKey) {
                    $adherenciaTiempoPorNombre[$nomKey][] = $val;
                }
            }
            ...
            if ($row->rmd !== null && is_numeric($row->rmd)) {
                $val = (float) $row->rmd;
                if ($docKey) {
                    $rmdPorDocumento[$docKey][] = $val;
                }
                if ($nomKey) {
                    $rmdPorNombre[$nomKey][] = $val;
                }
            }
            ...
            if ($row->adherencia_checklist_pre !== null) {
                ...
                if ($docKey) {
                    $checklistPrePorDocumento[$docKey][] = $val;
                }
                if ($nomKey) {
                    $checklistPrePorNombre[$nomKey][] = $val;
                }
            }
            ...
            if ($row->adherencia_checklist_post !== null) {
                ...
                if ($docKey) {
                    $checklistPostPorDocumento[$docKey][] = $val;
                }
                if ($nomKey) {
                    $checklistPostPorNombre[$nomKey][] = $val;
                }
            }
        }
    ...
    public function exportar(Request $request): StreamedResponse
    {
        ...
        $calcAusentismo = function ($row) use ($festivosService, $normStr): float {
            ...
            if (! in_array(trim((string) ($row->entro_1 ?? '')), $vacios, true)) {
                return 100.0;
            }
            if (! in_array(trim((string) ($row->entro_2 ?? '')), $vacios, true)) {
                return 100.0;
            }
            $fechaStr = trim((string) ($row->fecha ?? ''));
            if ($fechaStr !== '') {
                try {
                    if ($festivosService->esDomingoOFestivo(Carbon::parse($fechaStr))) {
                        return 100.0;
                    }
                } catch (\Throwable) {
                }
        ...
        foreach ($correcciones as $r) {
            if (! empty($r->identificacion)) {
                $marcacionesIdSet[$normStr($r->identificacion)] = true;
            }
            if (! empty($r->nombre_completo)) {
                $marcacionesNomSet[$normStr($r->nombre_completo)] = true;
            }
        }
        ...
        foreach ($eventosRaw as $row) {
            ...
            if ($row->adherencia_checklist_pre !== null) {
                $v = (float) $row->adherencia_checklist_pre;
                if ($dk) {
                    $clPrePorDoc[$dk][] = $v;
                }  if ($nk) {
                    $clPrePorNom[$nk][] = $v;
            ...
            if ($row->adherencia_checklist_post !== null) {
                $v = (float) $row->adherencia_checklist_post;
                if ($dk) {
                    $clPostPorDoc[$dk][] = $v;
                } if ($nk) {
                    $clPostPorNom[$nk][] = $v;
        ...
        foreach ($sacRaw as $row) {
            ...
            if (! empty($row->colaborador_id)) {
                $sacPorColId[$row->colaborador_id][] = $v;
            }
            if (! empty($row->responsable)) {
                $sacPorResp[$normStr($row->responsable)][] = $v;
            }
        }
        ...
        $getVals = function ($docMap, $nomMap) use ($normStr) {
            return function ($colaborador) use ($docMap, $nomMap, $normStr) {
                ...
                if ($v === null && ! empty($colaborador->codigo_qr_skap)) {
                    $v = $docMap[$normStr($colaborador->codigo_qr_skap)] ?? null;
                }
                if ($v === null && ! empty($colaborador->nombre_completo)) {
                    $v = $nomMap[$normStr($colaborador->nombre_completo)] ?? null;
                }
        ...
        foreach ($colaboradores as $colab) {
            ...
            if (! $enDpo && ! empty($colab->codigo_qr_skap)) {
                $enDpo = isset($dpoQrSafetySet[$normStr($colab->codigo_qr_skap)]);
            }
            if (! $enDpo && ! empty($colab->cedula)) {
                $enDpo = isset($dpoQrSafetySet[$normStr($colab->cedula)]) || isset($dpoNombresSet[$normStr($colab->cedula)]);
            }
            if (! $enDpo && ! empty($colab->nombre_completo)) {
                $enDpo = isset($dpoNombresSet[$normStr($colab->nombre_completo)]);
            }
            $pDpo = $enDpo ? 0.0 : 100.0;

            // Ausentismo
            $scsAus = $ausentismosPorColaboradorId[$colab->id] ?? null;
            if ($scsAus === null && ! empty($colab->cedula)) {
                $scsAus = $ausentismosPorIdentificador[$normStr($colab->cedula)] ?? null;
            }
            if ($scsAus === null && ! empty($colab->codigo_qr_skap)) {
                $scsAus = $ausentismosPorIdentificador[$normStr($colab->codigo_qr_skap)] ?? null;
            }
            $pAus = ! empty($scsAus) ? (in_array(0.0, $scsAus, true) ? 0.0 : 100.0) : null;

            // Malas Marcaciones
            $enMarc = false;
            if (! empty($colab->cedula)) {
                $enMarc = isset($marcacionesIdSet[$normStr($colab->cedula)]);
            }
            if (! $enMarc && ! empty($colab->codigo_qr_skap)) {
                $enMarc = isset($marcacionesIdSet[$normStr($colab->codigo_qr_skap)]);
            }
            if (! $enMarc && ! empty($colab->nombre_completo)) {
                $enMarc = isset($marcacionesNomSet[$normStr($colab->nombre_completo)]);
            }
     ...
     */
    public function toggleChecklist(Request $request): RedirectResponse
    {
    ...
    /**
     * Filtra por "el mes de $columna está en $meses", de forma compatible
     * con cualquier motor de base de datos. `whereIn(DB::raw('MONTH(...)'),
     * ...)` solo funciona en MySQL (SQLite no tiene la función MONTH) y por
     * eso nunca se pudo probar esta lógica con la suite de tests, que corre
     * contra SQLite.
     *
     ...
     */
    private function whereMonthIn($query, string $columna, array $meses)
    {
        return $query->where(function ($q) use ($columna, $meses) {
            foreach ($meses as $mes) {
                $q->orWhereMonth($columna, $mes);
            }
        });
    }
}
